...
3. まず軽い読み取り(`getDoc`)で存在有無を確認し、新規の可能性が高い場合のみ画面OCR
   (氏名・整理番号・チケット番号の抽出、詳細は後述)を実行
4. `runTransaction` でドキュメントを取得して最終判定
   - 存在しない → `{ scannedAt, rawText, serialNumber, ticketNumber, surname, givenName }` を作成
     → 「OK(緑)」を約1.2秒表示して自動でスキャン再開
   - 存在する → 「NG(赤)」+ 初回読み取り日時 + 整理番号/チケット番号(氏名は非表示)を表示
     → 手動確認待ち
...
5. 「最新の履歴を取得」→「Excelでダウンロード」で `livepocket-scan-history_YYYYMMDD.xlsx` が
   ダウンロードされる(列: No / 読み取り日時 / 氏 / 名 / 整理番号 / チケット番号 / QRの内容 / 識別ID)
...
## 画面OCR(氏名・整理番号・チケット番号の読み取り)

QRコード自体は暗号化・署名済みと思われる不透明な文字列のため、LivePocketチケット画面に
テキストとして表示されている「氏」「名」「整理番号」「チケット番号」は、カメラ映像を
[Tesseract.js](https://tesseract.projectnaptha.com/)(ブラウザ内で完結する無料OCR、CDNから実行時に読み込み)
でOCRして取得しています。

- **重複判定のキーには使いません**(あくまでQRコードのハッシュ値で判定します)。OCR結果はあくまで
  「記録用の付加情報」という位置づけです
- 新規登録の可能性が高いスキャンの時だけOCRを実行します(明らかに重複と分かる場合はスキップし、
  受付のスピードを落とさないようにしています)
- OCRはベストエフォートです。画面の反射・照明・機種依存フォントなどにより、
  特に日本語の氏名は読み取れない/誤読することがあります。整理番号・チケット番号(英数字)は
  比較的安定して読み取れますが、100%ではありません
- NG(重複)画面には、個人情報保護の観点から**整理番号・チケット番号のみ**表示し、
  氏名は表示しません。管理画面(`/admin`、認証必須)では氏名も含めて確認できます
- OCRの実行には数秒かかることがあります(「判定中...」の表示が通常より長く続くのは正常です)
- ラベル文字(「整理番号」「チケット番号」等)を目印に値を抽出しているため、
  LivePocket側の画面デザインが変わると読み取れなくなる可能性があります。その場合は
  `src/lib/ocr.ts` の `parseAlphanumericFields` / `parseNameFields` 内の正規表現を調整してください

### 速度・精度向上のための工夫

- カメラ起動と同時にOCRエンジンの読み込みを裏で開始する(`warmUpOcrWorkers`)。
  実際にQRを読んだ瞬間の初回読み込み待ちをなくしている
- 整理番号・チケット番号(英数字)用と氏名(日本語)用でOCRワーカーを分け、**並列実行**する
  (直列に2回OCRするより体感速度が速い)
- 整理番号・チケット番号の認識文字は英数字のみに制限(`tessedit_char_whitelist`)し、
  紛らわしい誤認識(0とO、1とIなど)を減らしている
- 画像はグレースケール化+コントラスト強調してからOCRに渡している
- ページ分割モード(PSM)を「まとまったテキストブロック」向けに固定し、
  自動レイアウト解析の手間を省いている

これらは一般的なOCR精度向上策ですが、実際の効果は会場の照明・チケット画面の反射・
カメラ性能に左右されるため、実機での確認をおすすめします。さらに精度を上げたい場合は、
QRコードの検出位置を基準に「文字が書かれていそうな領域だけ切り出す」といった
追加の工夫も可能です(ただし、スタッフがカメラをどう構えるかに依存するため、
現状は導入していません)。
