...
- **通知音**: **どの端末でスキャンされた結果でも**、PC画面のスキャンログに新しい行が追加された瞬間に鳴ります
  (OK(通過)は控えめな音、NG(重複)・読み取りエラーは大きめの警告音)。
  PC自身のカメラでの判定だけでなく、スマホでの判定結果がログに反映された時にも鳴ります
  (Web Audio APIでその場生成しており、音声ファイルは使用していません)。
  ブラウザの自動再生制限により、ユーザー操作なしでは音声が鳴らないため、
  画面上部に**「🔊 タップして通知音を有効にする」ボタン**を設置しています。
  **運用開始前に必ずこのボタンを1回押してください**(押すと確認としてOK音が1回鳴り、
  ボタンの表示が「🔊 通知音: 有効です」に変わります)。押し忘れた場合の保険として、
  画面のどこかを1回クリックしても同様に有効化されます
...
- **画面スリープ・スクリーンセイバーの防止**: PC画面を開いている間、ブラウザのScreen Wake Lock APIを使って
  画面が自動的にスリープ/ロックされないようにしています(対応ブラウザ: Chrome/Edge等)。
  非対応ブラウザでは何も起きません(エラーにはなりません)。OS側の設定によっては、
  この機能だけでは完全に防げない場合があるため、その場合はWindows/macOS側の
  スリープ・スクリーンセイバー設定も合わせてご確認ください

## PC画面のスキャンログ(共有・永続化)

PC画面右側の「スキャンログ」は、**OK(通過)・NG(重複)・読み取りエラーのすべてがFirestoreに保存されており、
全端末で共有・リアルタイム更新**されます(`onSnapshot`によるリアルタイム購読のため、即座に反映されます)。
ページを再読み込みしても消えません。PC(母艦)は情報を集約する役割に特化しているため、
どの端末で発生したNG(重複読み取りの試み)もPC側でまとめて監視できます。

- OK分は `scans` コレクション、NG(重複)・読み取りエラーは `events` コレクションに保存されます
- それぞれ直近300件まで表示します
- この一覧取得(list)は認証なしでもできるようにしています(`firestore.rules`で1回のクエリ最大300件に制限)。
  QRの中身(意味のない文字列)と読み取り時刻・NG発生状況が誰でも閲覧できる状態になる点はご留意ください
- 管理画面の「全リセット」機能は、`scans`・`events`両方のコレクションと通過人数カウンターをまとめて削除します
